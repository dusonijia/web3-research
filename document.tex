...
Recent work by Li et al. \citep{li2024layoutlmv3} introduced LayoutLMv3, which combines textual and visual information for document understanding. Similarly, Huang et al. \citep{huang2024layoutllm} proposed LayoutLLM for instruction-following document understanding. While these approaches show promise, they primarily focus on single-document processing and lack the systematic knowledge extraction capabilities required for enterprise applications.

\subsection{Multi-Agent Systems in NLP}

Multi-agent systems have emerged as a powerful paradigm for complex NLP tasks. Zhao et al. \citep{zhao2025multia} demonstrated that LLM-based multi-agent systems improve information transparency and reliability in automated supply chain coordination. Bogachov and Zhao \citep{bogachov2025lightweight} developed a lightweight multi-expert generative language model system for engineering information extraction, achieving significant computational efficiency gains.

The collaborative potential of multi-agent systems is further evidenced by Zhang et al. \citep{zhang2024multiagent}, who proposed a framework for knowledge graph construction using specialized extraction and domain agents. Their approach achieved notable improvements in entity and relation extraction tasks, providing foundational insights for our system design.
...
Wang et al. \citep{wang2024survey} provided a comprehensive survey of LLMs in knowledge representation learning, highlighting the potential for "contextual distillation" to convert implicit LLM knowledge into explicit graph structures. This concept directly informs our knowledge extraction methodology.

\subsection{Intellectual Property Detection}

Automated IP detection represents a specialized application domain with unique challenges. Bai et al. \citep{bai2024patentgpt} introduced PatentGPT, a domain-specific language model fine-tuned for patent analysis. Aristodemou and Tietze \citep{aristodemou2024patent} explored patent landscape analysis using transformer models, demonstrating the feasibility of automated patent classification and prior art search.
...
The integration of reinforcement learning (RL) with NLP tasks has shown promising results in various applications. Ouyang et al. \citep{ouyang2022training} demonstrated the effectiveness of reinforcement learning from human feedback (RLHF) in aligning language models with human preferences. Schulman et al. \citep{schulman2017proximal} introduced Proximal Policy Optimization (PPO), which has become a standard algorithm for RL applications in NLP.

Recent work by Liu et al. \citep{liu2024reinforcement} explored RL-based prompt optimization for few-shot learning tasks, achieving significant performance improvements. Our RL-based prompt optimizer builds upon these foundations while addressing the specific challenges of document understanding tasks.
...
Table~\ref{tab:scalability} illustrates system performance under varying concurrent loads.
