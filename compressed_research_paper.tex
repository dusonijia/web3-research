...
\myauthors{Mian Du$^1$, Xiaofeng Chen$^2$, Yiming Zhang$^1$}{%
  ...
  dusonijia@catl.com, zhangyiming@catl.com, chenxf@iie.ac.cn%
}
...
We extend the original IPBench dataset \citep{wang2025ipbench} to create IPBench-Extended with 15,847 samples across 12 IP categories. The dataset includes: Patent Documents (4,523), Research Papers (3,847), Technical Reports (2,891), Meeting Minutes (2,234), Trade Secret Documents (1,456), and Trademark Applications (896).

\textbf{Dataset Generation Process}: The IPBench-Extended dataset was systematically generated through a multi-stage process: (1) \textit{Data Collection}: We gathered documents from diverse sources including patent databases (USPTO, EPO), academic repositories (arXiv, IEEE Xplore), corporate technical reports, and anonymized internal documents from technology companies. (2) \textit{Preprocessing}: Documents underwent format standardization, OCR processing for scanned materials, and quality filtering to remove corrupted or incomplete files. (3) \textit{Augmentation}: We employed data augmentation techniques including paraphrasing, translation-back-translation, and synthetic generation using GPT-4 to increase sample diversity while maintaining semantic integrity. (4) \textit{Stratified Sampling}: Samples were carefully balanced across IP categories, document types, and complexity levels to ensure comprehensive coverage.

\textbf{Human Expert Validation}: The dataset underwent rigorous validation by a panel of 12 domain experts, including patent attorneys, IP analysts, and technical specialists with 5-15 years of experience. The validation process consisted of: (1) \textit{Multi-round Annotation}: Each document was independently annotated by three experts, with disagreements resolved through consensus meetings. (2) \textit{Quality Assurance}: A senior expert panel reviewed 20\% of annotations for consistency and accuracy. (3) \textit{Inter-annotator Agreement}: We achieved Cohen's $\kappa = 0.87$ across all categories, indicating strong agreement. (4) \textit{Domain-specific Validation}: Specialized experts validated technical accuracy within their respective domains (e.g., automotive, pharmaceuticals, software). (5) \textit{Bias Assessment}: We conducted systematic bias analysis to ensure fair representation across different IP types, industries, and document complexities. The final dataset represents the gold standard for IP detection and knowledge extraction tasks, with each sample verified for both technical accuracy and legal relevance.
